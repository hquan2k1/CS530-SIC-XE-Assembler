...
void Pass1::handle_LTORG(string& litPrefix, int& lineNumberDelta,int lineNumber,int& LOCCTR, int& lastDeltaLOCCTR){
...
void Pass1::evaluateExpression(string expression, bool& relative,string& tempOperand,int lineNumber, ofstream& errorFile,bool& error_flag){
...
if(opcode=="START"){
    ...
    LOCCTR = startAddress;
    writeData = intToStringHex(LOCCTR-lastDeltaLOCCTR) + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + comment;
...
while(opcode!="END"){
    ...
    if(!checkCommentLine(fileLine)){
      ...
      if(opcode=="EQU" && SYMTAB[label].relative == 0){
        writeData = writeDataPrefix + "\t" + intToStringHex(LOCCTR-lastDeltaLOCCTR) + "\t" + " " + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + comment + writeDataSuffix;
      } 
      else{
        writeData = writeDataPrefix + "\t" + intToStringHex(LOCCTR-lastDeltaLOCCTR) + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + comment + writeDataSuffix;
  ...
  if(opcode!="END"){
     ...
     if(SYMTAB[label].exists=='n'){
  	...
  	writeToFile(intermediateFile, writeDataPrefix + intToStringHex(LOCCTR-lastDeltaLOCCTR) + "\t" + label + "\t" + opcode);
...
if(opcode=="END"){
  ...
  writeData = intToStringHex(LOCCTR-lastDeltaLOCCTR) + " " + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + comment + writeDataSuffix;
  writeToFile(intermediateFile,writeData);
